...
/- Intuitively, an inductive type is built up from a specified list of constructors. 
   ...
   Intuition: each constructor specifies a way of building new objects of type `foo`, possibly from previously 
   constructed values. The type `foo` consists of nothing more than the objects that are constructed in this way.
   The first character `|` is optional. Also, we could separate constructors using commas instead of bars.

   Arguments to constructors can include objects of type `foo`, subject to a certain "positivity" constraint, 
   which guarantees that elements of `foo` are built from the bottom up. Roughly speaking, each `...` can be 
   any Pi type constructed from `foo` and previously defined types, in which `foo` appears, if at all, only 
   as the "target" of the Pi type. For more details, see [Dybj94]_.

   Besides inductive types, we'll see generalizations: mutually defined inductive types and *inductive families*.
-/
#print "==========================================="
...
namespace Sec_7_1
  ...
  /- The elimination principle `weekday.rec` is defined with `weekday` and its constructors. 
     `weekday.rec` is also known as a recursor; it's what makes the type "inductive" and allows us to define 
     a function on weekday by assigning values corresponding to each constructor. Intuition: an inductive 
     type is exhaustively generated by its constructors, and has no elements beyond those they construct. -/

  -- We will use a slight variant of `weekday.rec`, `weekday.rec_on` (also generated automatically), 
  -- which takes its arguments in a more convenient order. (Note that the shorter names `rec` and `rec_on` 
  -- are not made available by default when we open the `weekday` namespace. This avoids clashes with the 
  -- functions of the same names for other inductive types.)
  ...
  -- It is useful to group related definitions and theorems in a single namespace. We can put `number_of_day` 
  -- in the `weekday` namespace and then use the shorter name when we open the namespace.

  -- `rec_on` and `cases_on` are generated automatically, but are protected to avoid name clashes. They're not 
  -- provided by default when a namespace is opened, but we can declare aliases for them with `renaming`
  ...
  namespace weekday   -- We can define functions from weekday to weekday:
    ...
    weekday.cases_on d 
      ...
      -- but `show` is just for clarity; we can just use `rfl` we do for the remaining cases.
      rfl rfl rfl rfl rfl
  ...
  namespace hide₁  -- use `hide₁` to avoid conflicts with stdlib.
