...
namespace Sec_7_7
  ...
  /-An inductive family is an indexed family of types defined by a simultaneous induction of 
    ...
    the more general version constructs a function `... → Sort u`, where "`...`" denotes a 
    sequence of argument types, also known as *indices*. Each constructor then constructs an 
    element of some member of the family. One example is the definition of `vector α n`, 
    the type of vectors of elements of `α` of length `n`:
  -/
  open nat
  universe u
  namespace hidden
    ...
    -- Notice that `cons` takes an element of `vector α n` and returns an element of 
    -- `vector α (succ n)`, thereby using an element of one member of the family to build an 
    -- element of another.
  ...
  /-For each fixed `α : Sort u` and `a : α`, this definition constructs a family of types 
    ...
    which is an element of `eq a a`. Intuitively, the only way to construct a proof of `eq a x` 
    is to use reflexivity, in the case where `x` is `a`. Note that ``eq a a`` is the only 
    inhabited type in the family of types `eq a x`.  -/
    
  -- The elimination principle generated by Lean is as follows:

  universes u v

  #check (@eq.rec_on : Π {α : Sort u} {a : α} {C : α → Sort v} {b : α}, a = b → C a → C b)

  -- It's remarkable that all the basic axioms for equality follow from the constructor `refl`
  -- and the eliminator `eq.rec_on`. The definition of equality is atypical, however; 
  -- see discussion in the next section.

  -- The recursor ``eq.rec_on`` is also used to define substitution:
  namespace hidden
    universe u
    inductive eq {α : Type u} (a : α) : α → Prop | refl : eq a
  ...
  -- Using the recursor with `h₁ : a = b`, we may assume `a` and `b` are the same, 
  -- in which case, `p b` and `p a` are the same. The definition of `subst` is marked with an 
  -- elaboration hint, as described in :numref:`elaboration_hints`.
  ...
  namespace hidden
    ...
    theorem trans {α : Type u} {a b c : α} (h₁ : eq a b) (h₂ : eq b c) : eq a c := sorry

    theorem congr {α β : Type u} {a b : α} (f : α → β) (h : eq a b) : eq (f a) (f b) := sorry
  end hidden
